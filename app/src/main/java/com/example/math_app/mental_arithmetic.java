...
import com.example.math_app.Adapter.TinhnhamAdapter;
...
public class mental_arithmetic extends AppCompatActivity {
    private TinhnhamAdapter tinhnhamAdapter;
    private List<Tinhnham> tinhnhamList;
    private DatabaseReference databaseReference;
    private Button bt_dapan1, bt_dapan2, bt_dapan3, bt_dapan4;
    private Button icon_true1, icon_false1, icon_true2, icon_false2, icon_true3, icon_false3, icon_true4, icon_false4, bt_dialog_back;
    ...
    private void checkAnswer(Button button, Button icon_true, Button icon_false){
        Tinhnham tinhnham = tinhnhamList.get(index);
        String dapan = tinhnham.getDapan();
        if (!checkClick){
            checkClick = true;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tinhnhamList = new ArrayList<>();
        tinhnhamAdapter = new TinhnhamAdapter(tinhnhamList);
        TinhnhamData();
        ...
        img_next.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!checkClick){
                    Toast.makeText(mental_arithmetic.this,"Chưa chọn đáp án", Toast.LENGTH_SHORT).show();
                }
                if(checkClick){
                    if(index < tinhnhamList.size()-1){
                        index++;
                        Tinhnham tinhnham = tinhnhamList.get(index);
                        Picasso.get().load(tinhnham.getHinhanh()).into(img_hinhanh);
                        tv_noidung.setText(tinhnham.getNoidung());
                        bt_dapan1.setText(tinhnham.getDapan1());
                        bt_dapan2.setText(tinhnham.getDapan2());
                        bt_dapan3.setText(tinhnham.getDapan3());
                        bt_dapan4.setText(tinhnham.getDapan4());
                        int a = index+1;
    ...
    private void TinhnhamData() {
        databaseReference.child("Tinhnham").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                tinhnhamList.clear();
                for (DataSnapshot dataSnapshot : snapshot.getChildren()){
                    Tinhnham tinhnham = dataSnapshot.getValue(Tinhnham.class);
                    tinhnhamList.add(tinhnham);
                }
                if(index < tinhnhamList.size()-1){
                    index++;
                    Tinhnham tinhnham = tinhnhamList.get(index);
    ...
    private void showDialog(){
        final Dialog dialog = new Dialog(mental_arithmetic.this);
        ...
        bt_back.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                startActivity(new Intent(mental_arithmetic.this, menu.class));
                dialog.dismiss();
            }
        });
        dialog.show();
