...
import com.example.math_app.Adapter.TapdemAdapter;
...
public class quesstion_count extends AppCompatActivity {
    private TapdemAdapter tapdemAdapter;
    ...
    private void checkAnswer(Button button, Button icon_true, Button icon_false){
        Tapdem tapdem = tapdemList.get(index);
        String dapan = tapdem.getDapan();
        if (!checkClick){
            checkClick = true;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tapdemList = new ArrayList<>();
        tapdemAdapter = new TapdemAdapter(tapdemList);
        Tapdemdata();
        ...
        img_next.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (!checkClick){
                    Toast.makeText(quesstion_count.this,"Chưa chọn đáp án", Toast.LENGTH_SHORT).show();
                }
                if(checkClick){
                    if(index < tapdemList.size()-1){
                        index++;
                        Tapdem tapdem = tapdemList.get(index);
                        Picasso.get().load(tapdem.getHinhanh()).into(img_hinhanh);
                        tv_noidung.setText(tapdem.getNoidung());
                        bt_dapan1.setText(tapdem.getDapan1());
                        bt_dapan2.setText(tapdem.getDapan2());
                        bt_dapan3.setText(tapdem.getDapan3());
                        bt_dapan4.setText(tapdem.getDapan4());
                        int a = index+1;
    ...
    private void Tapdemdata() {
        databaseReference.child("Tapdem").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                tapdemList.clear();
                for (DataSnapshot dataSnapshot : snapshot.getChildren()){
                    Tapdem tapdem = dataSnapshot.getValue(Tapdem.class);
                    tapdemList.add(tapdem);
                }
                if(index < tapdemList.size()-1){
                    index++;
                    Tapdem tapdem = tapdemList.get(index);
    ...
    private void showDialog(){
        final Dialog dialog = new Dialog(quesstion_count.this);
        ...
        bt_back.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                startActivity(new Intent(quesstion_count.this, menu.class));
                dialog.dismiss();
            }
        });
        dialog.show();
